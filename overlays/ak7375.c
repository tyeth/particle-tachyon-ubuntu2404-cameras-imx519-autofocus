...
// Patched for Qualcomm CCI bus compatibility (ak7375 on Tachyon QCM6490)
// Based on Arducam IMX519_AK7375 driver with CCI resilience patches.
...
static int ak7375_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl->id == V4L2_CID_FOCUS_ABSOLUTE) {
		...
		/* Lazy power-on: activate VCM on first use during streaming */
		if (!dev_vcm->powered_on) {
			ret = ak7375_i2c_write(dev_vcm, AK7375_REG_CONT,
					       AK7375_MODE_ACTIVE, 1);
			if (ret == 0) {
				dev_vcm->powered_on = true;
				dev_info(&client->dev, "AK7375 VCM activated\n");
				usleep_range(AK7375_CTRL_DELAY_US,
					     AK7375_CTRL_DELAY_US + 10);
			}
			/* Continue even if activation fails - the write below
			 * will report the real I2C error */
		}
...
{
	/*
	 * On Qualcomm CCI buses, skip pm_runtime_get_sync() which would
	 * try I2C when the bus isn't powered. The VCM will be activated
	 * lazily on first focus write during streaming.
	 */
	return 0;
}
...
static int ak7375_probe(struct i2c_client *client)
{
	...
	/* Skip pm_runtime_idle - triggers suspend with CCI I2C failures */
	pm_runtime_set_active(&client->dev);
	pm_runtime_enable(&client->dev);

	dev_info(&client->dev, "ak7375 VCM registered (focus 0-%d)\n",
...
static int __maybe_unused ak7375_vcm_suspend(struct device *dev)
{
	...
	if (ret)
		dev_dbg(dev, "standby I2C failure: %d (CCI may be off)\n", ret);

	ak7375_dev->powered_on = false;
	return 0;  /* Never propagate - CCI power is external */
}
...
static int __maybe_unused ak7375_vcm_resume(struct device *dev)
{
	...
	ret = ak7375_i2c_write(ak7375_dev, AK7375_REG_CONT,
			       AK7375_MODE_ACTIVE, 1);
	if (ret) {
		dev_dbg(dev, "activate I2C failure: %d (CCI may be off)\n", ret);
		return 0;  /* Don't propagate */
	}

	ak7375_dev->powered_on = true;
	return 0;
}

static const struct of_device_id ak7375_of_table[] = {
	{ .compatible = "asahi-kasei,ak7375" },
	{ .compatible = "asahi-kasei,ak7345" },
	{ /* sentinel */ }
};
...
MODULE_DESCRIPTION("AK7375 VCM driver (patched for Qualcomm CCI)");
